...
class TabManager(
    ...
    fun createTab(url: String = "about:blank", isPrivate: Boolean = false): String {
        ...
        _tabs.value = _tabs.value + TabState(entity)
        _selectedId.value = id
        return id
    }

    fun selectTab(id: String) { _selectedId.value = id }

    fun closeTab(id: String) {
        inspector?.clear(id)
        _tabs.value = _tabs.value.filterNot { it.entity.id == id }
    ...
    fun closePrivateTabs() {
        ...
        privateIds.forEach { inspector?.clear(it) }
        _tabs.value = _tabs.value.filterNot { it.entity.isPrivate }
    ...
    fun updateTabUrl(id: String, url: String, title: String) {
        _tabs.value = _tabs.value.map { if (it.entity.id == id) it.copy(entity = it.entity.copy(url = url, title = title, lastActiveAt = System.currentTimeMillis())) else it }
    }

    fun setInspectorEnabled(id: String, enabled: Boolean): Boolean {
        val current = _tabs.value.find { it.entity.id == id } ?: return false
        if (current.inspectorEnabled == enabled) return true
        val enabledCount = _tabs.value.count { it.inspectorEnabled }
        if (enabled && enabledCount >= maxConcurrentCaptureTabs) return false
        _tabs.value = _tabs.value.map { if (it.entity.id == id) it.copy(inspectorEnabled = enabled) else it }
        return true
    }
